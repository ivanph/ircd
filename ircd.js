...
User.prototype.join = function (channelToJoin) {
  var channelName = normalizeChannelName(channelToJoin);
...
User.prototype.parse = function (message) {
  ...
  switch (command) {
    ...
    case 'NAMES': {
      const args = rest.split(/\s/);
      const channelNames = args[0].split(',');
    ...
    case 'WHO': {
      ...
      const channelName = normalizeChannelName(args[0]);
      if (channels[channelName]) {
